...
class GeneticAtack(object):
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        new_x_preds = self.neighbour_model.predict(self.sess, np.array(new_x_list)) #REPLACE LE MOT PAR CHACUN DES MOTS LES PLUS PROBABLE ET GET LA PREDICT
        ...
        new_x_scores = new_x_preds[:, target] # Keep only the score of the target

        orig_score = self.model.predict(self.sess, x_cur[np.newaxis, :])[0, target]  # SCORE entre 0  et 1 pour la classe target. de la phrase originale
        
        new_x_scores = new_x_scores - orig_score # Imp0act du changement de mots sur le score
        # Eliminate not that clsoe words
        new_x_scores[self.top_n:] = -10000000 #GARDER LE SCORE DES TOP_N MOTS-1

        if self.use_lm:
            prefix = ""
            suffix = None
            if pos > 0:
                prefix = self.dataset.inv_dict[x_cur[pos-1]] # LE MOT D'AVANT
            #
            orig_word = self.dataset.inv_dict[x_orig[pos]]

            if self.use_suffix and pos < x_cur.shape[0]-1:
                if (x_cur[pos+1] != 0):
                    suffix = self.dataset.inv_dict[x_cur[pos+1]]  # LE MOT SUIVANT
            # print('** ', orig_word)
    ...
    def perturb(self, x_cur, x_orig, neigbhours, neighbours_dist,  w_select_probs, target):
        ...
        x_len = w_select_probs.shape[0] #LONGUEUR DE LA PHRASE
        ...
        replace_list = neigbhours[rand_idx] # LES VOISINS DU MOT QUON VEUT CHANGER

        # S'il y a pas assez de voisin, fill la liste pour etre de longueur top_n
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        neigbhours_list = [x[0] for x in tmp]  # liste des mots les plus proche en orde croissant POUR CHACUN DES MOTS
        neighbours_dist = [x[1] for x in tmp]  # distance du mots orginal de chancun des mots les plus semblables POUR CHACUN DES MOTS
        neighbours_len = [len(x) for x in neigbhours_list]  # longueur de chaque mot (nombre de lettre) NOMBRE DE MOTS TROUVÉ POUR CHACUN DES MOTS
        ...
        w_select_probs = neighbours_len / np.sum(neighbours_len) #Probibility higher if has more neighbours

        tmp = [glove_utils.pick_most_similar_words(
            x_orig[i], self.dist_mat, self.top_n, 0.5) for i in range(x_len)] # Top_n des mots les plus proches
